...
static int specialAction(afb_req_t req, char * input_name, json_object *resultJ) {
    ...
    else AFB_REQ_NOTICE(req, "Found env script path : %s/%s", scriptPathEnv, script_name);
...
void afv_info(afb_req_t req, unsigned argc, afb_data_t const argv[]) {
    ...
    if (infoArgJ == NULL || jerr != json_tokener_success) {
        afb_req_reply_string(req, AFB_ERRNO_INVALID_REQUEST, "failure while packing info() verb arguments");
        return;
    }
...
void afv_get(afb_req_t req, unsigned argc, afb_data_t const argv[]) {
	...
	if(api_ctx) {
		...
		switch (json_object_get_type(json_request)) {
			...
			default:
				afb_req_reply_string(req, AFB_ERRNO_INVALID_REQUEST, "Argument type is unknown, you must provide a string only");
				return;
		}
...
void afv_unsubscribe(afb_req_t req, unsigned argc, afb_data_t const argv[]) {
   ...
   if (!client_ctx) {
      afb_req_reply_string(req, AFB_ERRNO_INVALID_REQUEST, "No context available for the client.");
      return;
   }
